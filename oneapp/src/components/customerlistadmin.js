...
class customerlistadmin extends Component {
  ...
  getcustomerListforAdmin = () => {
    ...
    ApiService.getcustomerListforAdmin()
      .then((response) => {
        const { data } = response;
        console.log(response);
        console.log(data);
  ...
  render() {
    const { error, isLoaded, customerslist, handleAdd, flag } = this.state;
    if (error) {
...
export default connect(mapStateToProps, mapActionsToProps)(customerlistadmin);
